...
client.on('message', async msg => {
    ...
    try {
        // Admin commands dijalankan dulu (bisa paralel dengan user flow)
        if (isAdmin(phone) && body.startsWith('!')) {
            await handleAdmin(msg, phone, body, lower);
            return; // admin commands tidak dilanjut ke user handler
        }
...
📝 Note     : ${akun.note || '-'}`
        ...
        // ── !setqris (kirim gambar + caption !setqris) ────────
        case '!setqris': {
            if (msg.hasMedia) {
                try {
                ...
                await msg.reply('📸 Kirim gambar QRIS dengan caption *!setqris*');
            }
            break;
        }
...
async function handleUser(msg, phone, body, lower) {
    ...
    switch (state.step) {
        ...
        case 'input_password': {
            ...
            await msg.reply(msgKonfirmasiOrder(d));
            break;
        }
        ...
        case 'confirm_order': {
            if (['ya', 'yes', 'iya', 'ok', 'y', 'confirm', 'setuju'].includes(lower)) {
                ...
                resetState(phone);
                await msg.reply(msgOrderSubmitted(d.order_id));

                // Notifikasi admin
                await notifyAdminOrder(d, fmt(phone));
