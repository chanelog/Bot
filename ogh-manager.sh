#!/bin/bash
# ╔══════════════════════════════════════════════════════════════════╗
# ║           OGH-UDP ALL-IN-ONE MANAGER v3.1                       ║
# ║  Synced with: ogh_bot.py + install_bot.sh                       ║
# ║  OGH UDP  : github.com/chanelog/Ogh/raw/main/udpServer          ║
# ║  ZivPN UDP: github.com/fauzanihanipah/ziv-udp                   ║
# ╚══════════════════════════════════════════════════════════════════╝

# ─── VERSION (harus sama di ketiga file) ───────────────────────────
VERSION="3.1"
SYNC_KEY="OGH-SYNC-3.1"   # dipakai bot & installer untuk validasi
...
# ══════════════════════════════════════════════════════════════════
#  ▼▼▼  BLOK KONFIGURASI PUSAT — sinkron dengan ogh_bot.py  ▼▼▼
# ══════════════════════════════════════════════════════════════════
# --- OGH-UDP ---
...
# --- Bot (diinstall oleh install_bot.sh) ---
BOT_DIR="/opt/ogh-bot"
...
bot_running() { systemctl is-active --quiet $BOT_SVC 2>/dev/null; }
...
bot_stlabel() { bot_running && echo -e "${G}● RUNNING${NC}" || echo -e "${R}● STOPPED${NC}"; }
...
reset_quota_usage() {
    echo 0 > "$(quota_file "$1" "$2")"
    sed -i "s/^${2}|\([^|]*\)|\([^|]*\)|\([^|]*\)|\([^|]*\)|\([^|]*\)|\([^|]*\)|/${2}|\1|\2|\3|\4|\5|0|/" "$3" 2>/dev/null
    ok "Quota '$2' direset."
}
reset_sessions() { echo 0 > "$(session_file "$1" "$2")"; }
...
# ══════════════════════════════════════════════════════════════════
#  AUTO INSTALL — jalankan setiap kali script dieksekusi
# ══════════════════════════════════════════════════════════════════
auto_install() {
    clear
    echo -e "${C}${BOLD}"
...
JSON
    ...
    # ── Bot DB dir (dibuat juga di sini agar sinkron) ─────────────
    mkdir -p "$BOT_DB_DIR"
    [ ! -f "$BOT_RESELLER_DB" ] && echo '{}' > "$BOT_RESELLER_DB"
...
BOTCFG
        ok "Bot config siap ($BOT_CFG_FILE)."
    fi
...
show_header() {
    ...
    echo -e "  ${C}OS         :${NC} $OS"
    echo -e "  ${C}Kernel     :${NC} $(get_kernel)    ${C}Uptime  :${NC} $UPTIME"
    echo -e "  ${C}CPU        :${NC} $CPU             ${C}Load    :${NC} $LOAD"
    echo -e "  ${C}RAM        :${NC} $RAM"
    echo -e "  ${C}Disk       :${NC} $DISK"
    sline
    echo -e "  ${C}OGH-UDP  ${NC} Port:${W}$(ogh_port)${NC}  $(ogh_stlabel)  Akun:${W}$(count_db "$OGH_DB")${NC}"
    echo -e "  ${C}ZivPN-UDP${NC} Port:${W}$(ziv_port)${NC}  $(ziv_stlabel)  Akun:${W}$(count_db "$ZIV_DB")${NC}"
    echo -e "  ${C}Bot TG   ${NC} $(bot_stlabel)  Log: $BOT_LOG"
    ...
    echo -e "  ${G}[9]${NC}  Reset Kuota Usage      ${G}[10]${NC} Reset Session"
    ...
    echo -e "  ${G}[29]${NC} Reset Kuota Usage      ${G}[30]${NC} Reset Session"
    ...
    echo ""
    echo -e "  ${M}${BOLD}◆ Telegram Bot${NC}"
    sline
    echo -e "  ${Y}[71]${NC} Install Bot TG         ${Y}[72]${NC} Start Bot TG"
    echo -e "  ${Y}[73]${NC} Stop Bot TG            ${Y}[74]${NC} Restart Bot TG"
    echo -e "  ${C}[75]${NC} Status Bot TG          ${C}[76]${NC} Log Bot TG"
    echo -e "  ${C}[77]${NC} Edit Token/Admin Bot   ${C}[78]${NC} Update Bot TG (GitHub)"
    echo ""
    echo -e "  ${M}${BOLD}◆ Monitoring & Tools${NC}"
    sline
    echo -e "  ${G}[51]${NC} Monitor Live OGH       ${G}[52]${NC} Monitor Live ZivPN"
    echo -e "  ${G}[53]${NC} Statistik Akun OGH     ${G}[54]${NC} Statistik Akun ZivPN"
    echo -e "  ${G}[55]${NC} Cek Akun Mau Expired   ${G}[56]${NC} Backup Database"
    echo -e "  ${G}[57]${NC} Restore Database       ${G}[58]${NC} Export Akun ke TXT"
    echo ""
...
_create_user() {
    ...
    [[ "$U" =~ [^a-zA-Z0-9_] ]] && err "Hanya huruf, angka, underscore." && return
    read -s -p "  Password        : " P; echo
    [ -z "$P" ] && err "Password kosong." && return
    read -p "  Expired (hari)  [30] : " D;  D=${D:-30}
    read -p "  Max Login       [0=∞]: " ML; ML=${ML:-0}
    read -p "  Kuota Data      [cth: 10GB, 0=∞]: " QT
    ...
    line; ok "Akun berhasil dibuat!"; line
...
_list_users_inline() {
    ...
    echo ""
    printf "  ${C}%-3s %-15s %-11s %-8s %-12s %-10s %-10s %s${NC}\n" \
        "No" "Username" "Expired" "MaxLogin" "Kuota" "Terpakai" "Sesi" "Status"
    sline
    ...
    while IFS='|' read -r U P EXP CR ML QB USED ST; do
        ...
        printf "  %-3s %-15s " "$N" "$U"
        echo -ne "$EXP_C  "
        printf "%-8s %-12s %-10s %-10s " "$ML_L" "$QB_H" "$USED_H" "$SESS"
        echo -e "$ST_C"
        ((N++))
    done < "$DB"
    echo ""
...
_list_users() {
    ...
    _list_users_inline "$DB" "$QDIR" "$SDIR"
    line; echo -e "  Total: ${W}$(count_db "$DB")${NC} akun"
}
...
_check_user() {
    ...
    sline
    echo -e "  ${C}Max Login   :${NC} $ML_L"
    echo -e "  ${C}Sesi Aktif  :${NC} $SESS / $ML_L"
    sline
...
_set_quota() {
    ...
    read -p "  Kuota baru [cth: 10GB, 0=unlimited]: " NQ
...
ogh_change_port() {
    ...
    read -p "  Port baru: " NP
    [[ ! "$NP" =~ ^[0-9]+$ ]] || [ "$NP" -lt 1 ] || [ "$NP" -gt 65535 ] \
        && err "Port tidak valid." && return
    echo "$NP" > "$OGH_PORT_FILE"
    ok "Port OGH-UDP diubah ke $NP."
    read -p "  Restart sekarang? (y/n): " R; [ "$R" = "y" ] && ogh_restart
}
ogh_log() { title "LOG OGH-UDP (60 baris)"; tail -n 60 "$OGH_LOG" 2>/dev/null || warn "Log kosong."; line; }
...
ziv_reset_cfg() {
    ...
    if [ "$C" = "yes" ]; then
        _dl "$ZIV_CFG_URL" "$ZIV_CFG"; ok "config.json direset."
        read -p "  Restart? (y/n): " R; [ "$R" = "y" ] && ziv_restart
    else
        warn "Dibatalkan."
    fi
...
ziv_log() { title "LOG ZivPN-UDP (60 baris)"; tail -n 60 "$ZIV_LOG" 2>/dev/null || warn "Log kosong."; line; }

# ══════════════════════════════════════════════════════════════════
#  TELEGRAM BOT MANAGEMENT (sinkron dengan install_bot.sh)
# ══════════════════════════════════════════════════════════════════
bot_install() {
    title "INSTALL TELEGRAM BOT"
    info "Mengunduh install_bot.sh dari GitHub..."
    local TMP_INST; TMP_INST=$(mktemp)
    _dl "$INSTALL_URL" "$TMP_INST"
    if [ -s "$TMP_INST" ]; then
        bash "$TMP_INST"
    else
        err "Gagal unduh install_bot.sh"
    fi
    rm -f "$TMP_INST"
}
...
bot_start()   {
    if [ ! -f "$BOT_PY" ]; then
        err "Bot belum diinstall. Gunakan [71] untuk install dulu."; return
    fi
    systemctl start $BOT_SVC; sleep 1
    bot_running && ok "Bot TG berjalan." || err "Gagal start bot. Cek log [76]."
}
bot_stop()    { systemctl stop    $BOT_SVC; warn "Bot TG dihentikan."; }
bot_restart() {
    if [ ! -f "$BOT_PY" ]; then
        err "Bot belum diinstall."; return
    fi
    systemctl restart $BOT_SVC; sleep 1
    bot_running && ok "Bot TG direstart." || err "Gagal restart.";
}
bot_status()  { systemctl status $BOT_SVC --no-pager; }
bot_log()     { title "LOG TELEGRAM BOT (60 baris)"; tail -n 60 "$BOT_LOG" 2>/dev/null || warn "Log kosong."; line; }

bot_edit_config() {
    title "EDIT TOKEN / ADMIN BOT"
    if [ ! -f "$BOT_PY" ]; then
        err "Bot belum diinstall di $BOT_PY"; return
    fi
    echo -e "  File: ${W}$BOT_PY${NC}"
    echo ""
    read -p "  Token baru (kosong=skip): " TOK
    read -p "  Admin ID baru (kosong=skip): " AID
    if [ -n "$TOK" ]; then
        sed -i "s|BOT_TOKEN    = \"[^\"]*\"|BOT_TOKEN    = \"$TOK\"|" "$BOT_PY"
        ok "Token diperbarui."
    fi
    if [ -n "$AID" ]; then
        sed -i "s|ADMIN_IDS    = \[[^]]*\]|ADMIN_IDS    = [$AID]|" "$BOT_PY"
        ok "Admin ID diperbarui."
    fi
    read -p "  Restart bot sekarang? (y/n): " R
    [ "$R" = "y" ] && bot_restart
}

bot_update() {
    title "UPDATE BOT DARI GITHUB"
    info "Mengunduh ogh_bot.py terbaru..."
    local TMP; TMP=$(mktemp)
    _dl "$BOT_PY_URL" "$TMP"
    if [ -s "$TMP" ]; then
        # Simpan token & admin ID dari file lama
        local OLD_TOK OLD_AID
        OLD_TOK=$(grep 'BOT_TOKEN' "$BOT_PY" 2>/dev/null | cut -d'"' -f2)
        OLD_AID=$(grep 'ADMIN_IDS' "$BOT_PY" 2>/dev/null | grep -oP '\[\K[^\]]+')
        cp "$TMP" "$BOT_PY"
        chmod +x "$BOT_PY"
        # Restore token & admin ID
        [ -n "$OLD_TOK" ] && sed -i "s|BOT_TOKEN    = \"[^\"]*\"|BOT_TOKEN    = \"$OLD_TOK\"|" "$BOT_PY"
        [ -n "$OLD_AID" ] && sed -i "s|ADMIN_IDS    = \[[^]]*\]|ADMIN_IDS    = [$OLD_AID]|" "$BOT_PY"
        ok "Bot diperbarui dari GitHub. Token & Admin ID dipertahankan."
        read -p "  Restart bot sekarang? (y/n): " R
        [ "$R" = "y" ] && bot_restart
    else
        err "Gagal mengunduh bot update."
    fi
    rm -f "$TMP"
}
...
stats_all_users() {
    ...
    while IFS='|' read -r U P EXP CR ML QB USED ST; do
        ...
        TOTAL_USED=$(( TOTAL_USED + $(get_used "$QDIR" "$U") ))
    done < "$DB" 2>/dev/null
...
backup_db() {
    ...
    cp "$BOT_RESELLER_DB" "$BK/resellers_$TS.json" 2>/dev/null
    ok "Backup disimpan di $BK/"; ls -lh "$BK/" | tail -6
}
...
export_users() {
    ...
    {
        echo "=================================="
        echo "  OGH-UDP ACCOUNT EXPORT v$VERSION"
        echo "  Tanggal: $(date)"
        echo "=================================="
        echo ""
        ...
        while IFS='|' read -r U P EXP CR ML QB _ ST; do
            ...
            echo "Username : $U | Pass : $P | Host : $IP | Port : $(ogh_port)"
        ...
        while IFS='|' read -r U P EXP CR ML QB _ ST; do
            ...
            [ "$QB" = "0" ] && QB_H="Unlimited" || QB_H=$(bytes_human "$QB")
            [ "$ML" = "0" ] && ML_L="Unlimited" || ML_L="$ML"
            echo "Username : $U | Pass : $P | Host : $IP | Port : $(ziv_port)"
            echo "Expired  : $EXP | MaxLogin : $ML_L | Kuota : $QB_H | Status : $ST"
...
sys_info() {
    ...
    echo -e "  ${C}Bot TG     :${NC} $BOT_PY  $(ls -lh "$BOT_PY" 2>/dev/null | awk '{print "("$5")"}')"
    echo -e "  ${C}OGH Svc    :${NC} $(ogh_stlabel)"
    echo -e "  ${C}ZivPN Svc  :${NC} $(ziv_stlabel)"
    echo -e "  ${C}Bot Svc    :${NC} $(bot_stlabel)"
    line
}
...
check_version() {
    ...
    echo ""
    echo -e "  ${C}Bot TG   :${NC} $BOT_PY"
    ls -lh "$BOT_PY" 2>/dev/null | awk '{print "  Size:"$5"  Modified:"$6" "$7}'
    line
}
...
while true; do
    ...
    case "$CH" in
        ...
        # Telegram Bot
        71) bot_install ;;    72) bot_start ;;
        73) bot_stop ;;       74) bot_restart ;;
        75) bot_status ;;     76) bot_log ;;
        77) bot_edit_config ;; 78) bot_update ;;
