...
const DOMAIN_PATH   = '/etc/zivpn/domain.conf';
const PORT          = '5667';
...
function getDomain() {
    try {
        return fs.readFileSync(DOMAIN_PATH, 'utf8').trim();
    } catch (e) {
        return 'N/A';
    }
}
...
function createUser(username, password, days, quota, note) {
    try {
        ...
        const users = readUsers();
        users.push({ username, password, exp, quota: quota || DEFAULT_QUOTA, note: note || '' });
        writeUsers(users);
...
function renewUser(username, days) {
    try {
        ...
        if (idx === -1) return { success: false, error: `Username ${username} tidak ditemukan.` };
